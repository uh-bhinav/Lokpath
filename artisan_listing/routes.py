...
from shared_globals import reverse_geocode, extract_simplified_region, extract_state_city_from_google 
import datetime
import re
...
def create_artisan_bp(db_instance): # Function to create and return the blueprint
    ...
    def submit_artisan_details():
        ...
        session_ref = db_instance.collection('submission_sessions').document(session_id)
        ...
        if session_doc.to_dict().get("upload_type") != "artisans":
            return jsonify({"error": "Session ID is not for an artisan listing."}), 400
        ...
        update_data = {
            ...
            "tags": tags,
            "listed_by_uid": user_uid,
            "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "type": "artisan_listing" # Keep for differentiating session types
        }
    ...
    def finalize_artisan_listing(session_id):
        ...
        doc = session_ref.get()
        if not doc.exists:
            return jsonify({"error": "Artisan listing session not found."}), 404
        
        data = doc.to_dict()
        # Ensure session is valid and explicitly for an artisan listing
        if data.get("type") != "artisan_listing" or data.get("listed_by_uid") != user_uid:
            return jsonify({"error": "Unauthorized or invalid session type."}), 403
